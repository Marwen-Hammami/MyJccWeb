...
class VoteRepository extends ServiceEntityRepository
{
    ...
    public function getRatePerGenre()
    {
        ...
        FROM film f
        INNER JOIN rate v ON f.ID_film = v.ID_Film
        CROSS JOIN (SELECT SUM(valeur) AS total FROM rate) total_votes
        GROUP BY f.genre";
    ...
    public function getVotesPerFilm()
    {
        ...
        $sql = "SELECT f.Titre as film_title, COUNT(v.ID_Vote) as num_votes
            FROM Film f
            JOIN Rate v ON f.ID_film = v.ID_Film
            WHERE v.Vote_Film = 1
            GROUP BY f.Titre";
